...
{{< expand "余談： さらなる構造化">}}
この例だと、さすがに `updateGame()` の中身が長くなってきたので、そこは改善の余地があるかもしれません（一画面に収まらなくなったら要改善、などと言われることも多いです）。

単純に複数の関数に分割するか、もしくは「エンティティグループ」のような概念を新たに作り、それに「エンティティの配列」と「更新の手順」の情報を持たせ、よしなに管理してもらう、といった方法などが考えられます。
{{< /expand >}}